...
class Supervisor {
    ...
    public function getMiCamino($nombreEmpleado) {
        try {
            $query = "SELECT 
                      ...
                      FROM LosSupervisores s
                      INNER JOIN LosCaminos c ON s.numCamino = c.numCamino
                      LEFT JOIN LasJaulas j ON c.numCamino = j.numCamino
                      LEFT JOIN LosAnimales a ON j.numJaula = a.numJaula
                      LEFT JOIN LosGuardas g ON j.numJaula = g.numJaula
    ...
    public function getJaulasCamino($nombreEmpleado) {
        try {
            $query = "SELECT 
                      ...
                      FROM LosSupervisores s
                      INNER JOIN LasJaulas j ON s.numCamino = j.numCamino
    ...
    public function getPersonalCamino($nombreEmpleado) {
        try {
            $query = "SELECT DISTINCT
                      ...
                      FROM LosSupervisores s
                      INNER JOIN LasJaulas j ON s.numCamino = j.numCamino
                      INNER JOIN LosGuardas g ON j.numJaula = g.numJaula
                      INNER JOIN LosEmpleados e ON g.nombreEmpleado = e.nombreEmpleado
                      LEFT JOIN LosAnimales a ON j.numJaula = a.numJaula
                      WHERE s.nombreEmpleado = :nombreEmpleado
    ...
    public function getDistribucionEspecies($nombreEmpleado) {
        try {
            $query = "SELECT 
                        a.nombreA AS especie,
                        COUNT(*) AS cantidad,
                        GROUP_CONCAT(DISTINCT j.nombre ORDER BY j.nombre SEPARATOR ', ') AS jaulas
                      FROM LosSupervisores s
                      INNER JOIN LasJaulas j ON s.numCamino = j.numCamino
                      INNER JOIN LosAnimales a ON j.numJaula = a.numJaula
                      WHERE s.nombreEmpleado = :nombreEmpleado
                      GROUP BY a.nombreA
